...
function pipelineSearchHealthStatus() {
  ...
  const officialSources = readPipelineEnvList(
...
function collectPipelineFocusInventory(db, channels) {
  const brand = readPipelineEnvString('AUTO_SEED_BRAND_FILTER', '')
...
async function inspectPublicPost(row) {
  ...
  if (channel === 'selfhosted' && issues.length > 0 && cacheCheckNeeded && url && !url.includes('?')) {
    ...
    try {
      ...
      // 원본 URL 실패 원인을 가리지 않기 위해 캐시 우회 확인 실패는 보조 신호로만 둔다.
    }
  }
  ...
  try {
    ...
    ok(res, {
      ...
      ops: {
        ...
        snapshot_source: 'local_sqlite_direct',
